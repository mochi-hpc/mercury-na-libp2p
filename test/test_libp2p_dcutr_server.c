/**
 * Test: DCUtR hole-punch verification — server side
 *
 * Identical to the relay server test. Initializes with relay transport,
 * writes its circuit address to a file, exchanges messages with the client.
 *
 * The test driver kills the relay between lookup and messaging.  If DCUtR
 * succeeded, the direct connection keeps working and messages flow.
 *
 * 1. Initialize as listener with relay transport
 * 2. Write circuit address to file for client
 * 3. Post unexpected recv, wait for client message
...
main(int argc, char *argv[])
{
    ...
    ret = NA_Addr_to_string(state.na_class, addr_str, &addr_str_size, self_addr);
    if (ret != NA_SUCCESS) {
    ...
    if (strncmp(state.recv_buf, "Hello from dcutr client", 23) != 0) {
    ...
    {
        const char *reply = "Reply from dcutr server";
        op_id = NA_Op_create(state.na_class, NA_OP_SINGLE);
